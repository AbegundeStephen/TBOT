...
import logging
import time  # ✅ ADDED: Required for retry logic
...
class BinanceFuturesHandler:
    ...
    def _open_position(
        ...
        """
        Internal method to open LONG or SHORT position with NAKED POSITION PROTECTION.
        ...
        Returns:
            Order response dict OR None if failed/emergency closed
        """
        try:
            ...
            # 2. CRITICAL: Place Stop Loss with Retries & Failsafe
            if stop_loss:
                sl_success = False
                ...
                for attempt in range(1, 4):
                    try:
                        ...
                        logger.info(f"  ✓ SL Placed: ${stop_loss:,.2f}")
                        sl_success = True
                ...
                # 3. EMERGENCY FAILSAFE: If SL failed 3 times, CLOSE THE POSITION
                if not sl_success:
                    logger.critical(f"[FUTURES] 🛑 CRITICAL: SL FAILED 3x! PERFORMING EMERGENCY CLOSE.")
                    ...
                    try:
                        ...
                        logger.critical(f"[FUTURES] ✓ EMERGENCY CLOSE SUCCESSFUL. Position neutralized.")
                    except Exception as close_error:
                        logger.critical(f"[FUTURES] ☠️ EMERGENCY CLOSE FAILED: {close_error}. MANUAL INTERVENTION REQUIRED!")
                    
                    # Return None so the bot knows the trade failed
                    return None
            
            # 4. Place Take Profit (Optional - Non-Critical)
            if take_profit:
                try:
                    ...
                    logger.info(f"  ✓ TP Placed: ${take_profit:,.2f}")
                except Exception as e:
                    logger.warning(f"  ⚠️ TP Failed (Non-critical): {e}")
            
            return order
            
        except Exception as e:
            logger.error(f"[FUTURES] Failed to execute _open_position logic: {e}")
            return None
