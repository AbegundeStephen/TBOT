...
class MT5ExecutionHandler:
    ...
    def __init__(self, config: Dict, portfolio_manager):
        ...
        # Auto-sync on initialization if enabled in config
        if self.trading_config.get("auto_sync_on_startup", True):
    ...
    def execute_signal(self, signal: int, symbol: str = None, asset: str = "GOLD") -> bool:
        ...
        try:
            ...
            if existing_position:
                ...
                logger.debug(f"{asset}: Existing {position_side.upper()} position found, signal={signal}")

                # === CLOSE POSITION SCENARIOS ===
                
                # Close long position on SELL signal
                if signal == -1 and position_side == 'long':
                    logger.info(f"[SIGNAL] {asset}: SELL signal received - Closing LONG position")
                ...
                # Close short position on BUY signal (if you trade shorts)
                    ...
                    logger.info(f"[SIGNAL] {asset}: BUY signal received - Closing SHORT position")
                ...
                # HOLD signal - check SL/TP
                    ...
                    if should_close:
                        ...
                        if not success:
                            logger.error(f"[FAIL] Failed to close {asset} position on {close_reason}")
                            return False
                    else:
                ...
                # CRITICAL FIX: Prevent duplicate positions
                    ...
                    logger.info(f"[SKIP] {asset}: BUY signal ignored - Already have LONG position")
                    return True  # Not an error, just skip
                
                elif signal == -1 and position_side == 'short':
                    logger.info(f"[SKIP] {asset}: SELL signal ignored - Already have SHORT position")
                    return True  # Not an error, just skip
            ...
            if signal == 1:  # BUY signal
                ...
                if self.portfolio_manager.has_position(asset, side='long'):
                    logger.warning(f"[SKIP] {asset}: BUY signal - Position already exists (double-check)")
                    return True
    ...
    def _close_mt5_position(self, position, current_price: float, asset: str, reason: str) -> bool:
        ...
        try:
            ...
            if mt5_position is None:
                logger.warning(f"{asset}: MT5 {side} position not found, closing in portfolio only")
                self.portfolio_manager.close_position(
                    asset=asset,
                    exit_price=current_price,
                    reason=reason
            ...
            order_type = mt5.ORDER_TYPE_SELL if mt5_position.type == mt5.POSITION_TYPE_BUY else mt5.ORDER_TYPE_BUY
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                logger.info(f"[OK] {asset} {side.upper()} position closed on MT5 and portfolio")
                return True
            else:
                error_msg = result.comment if result else 'No result'
                error_code = result.retcode if result else 'N/A'
                logger.error(f"[FAIL] Failed to close MT5 position: {error_msg} (code: {error_code})")
    ...
    def _open_mt5_position(self, signal: int, current_price: float, symbol: str, asset: str) -> bool:
        ...
        try:
            ...
            can_open, reason = self.portfolio_manager.can_open_position(asset, 'long' if signal == 1 else 'short')
            if not can_open:
            ...
            logger.info(f"[OK] {'BUY' if signal == 1 else 'SELL'} {asset} - Position opened successfully")
            return True
    ...
    def _emergency_close_mt5_position(self, symbol: str, volume: float, original_order_type: int):
        """Emergency close of an unwanted MT5 position"""
        try:
            close_order_type = mt5.ORDER_TYPE_SELL if original_order_type == mt5.ORDER_TYPE_BUY else mt5.ORDER_TYPE_BUY
            tick = mt5.symbol_info_tick(symbol)
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                logger.error(f"[FAIL] Emergency close failed: {result.comment if result else 'No result'}")
        except Exception as e:
            logger.error(f"Emergency close error: {e}", exc_info=True)

    def calculate_position_size(self, current_price: float, asset: str = "GOLD") -> Tuple[float, float]:
        """
        ...
        logger.debug(f"{asset}: Calculated {volume_lots:.2f} lots = ${actual_position_size:,.2f}")
    ...
    def sync_positions_with_mt5(self, asset: str = "GOLD", symbol: str = None) -> bool:
        ...
        try:
            ...
            # Case 1: MT5 has position, portfolio doesn't - IMPORT position
            if mt5_positions and len(mt5_positions) > 0 and not portfolio_position:
                mt5_pos = mt5_positions[0]
                
                # Extract position details
                position_type = "long" if mt5_pos.type == mt5.POSITION_TYPE_BUY else "short"
                ...
                contract_size = self.symbol_info.trade_contract_size
                position_size_usd = volume * entry_price * contract_size
                
                logger.warning(
                    f"[SYNC] Found MT5 {position_type.upper()} position for {asset}:\n"
                    f"  Entry: ${entry_price:.2f}\n"
                    f"  Volume: {volume:.2f} lots\n"
                    f"  Size: ${position_size_usd:,.2f}\n"
                    f"  Current P&L: ${profit:.2f}\n"
                    f"  → Adding to portfolio..."
                )
                
                # Calculate SL/TP from MT5 position (if set)
                stop_loss = mt5_pos.sl if mt5_pos.sl != 0 else None
                take_profit = mt5_pos.tp if mt5_pos.tp != 0 else None
                
                # If SL/TP not set in MT5, calculate defaults
                ...
                trailing_stop_pct = self.asset_config.get("risk", {}).get("trailing_stop_pct", 0.02)
                ...
                if success:
                    logger.info(f"[SYNC] ✓ Successfully imported {asset} position to portfolio")
                    return True
                else:
                    logger.error(f"[SYNC] ✗ Failed to add {asset} position to portfolio")
                    return False
            
            # Case 2: Portfolio has position, MT5 doesn't - REMOVE from portfolio
            elif portfolio_position and (not mt5_positions or len(mt5_positions) == 0):
                logger.warning(
                    f"[SYNC] Portfolio has {asset} position but MT5 doesn't\n"
                    f"  → Removing from portfolio (position was likely closed manually)"
                )
                current_price = self.get_current_price()
                ...
                if trade_result:
                    ...
                    logger.error(f"[SYNC] ✗ Failed to remove {asset} position")
                    return False
            
            # Case 3: Both have positions - VERIFY they match
            elif mt5_positions and len(mt5_positions) > 0 and portfolio_position:
                mt5_pos = mt5_positions[0]
                mt5_type = "long" if mt5_pos.type == mt5.POSITION_TYPE_BUY else "short"
                portfolio_side = portfolio_position.side if hasattr(portfolio_position, 'side') else portfolio_position.get('side')
                
                if mt5_type != portfolio_side:
                    logger.error(
                        f"[SYNC] MISMATCH: MT5 has {mt5_type.upper()} but portfolio has {portfolio_side.upper()}\n"
                        f"  → Closing portfolio position and re-importing from MT5"
                    )
                    
                    # Close incorrect portfolio position
                    current_price = self.get_current_price()
                    self.portfolio_manager.close_position(asset, current_price, "sync_mismatch")
                    
                    # Re-import from MT5 (recursive call)
                    return self.sync_positions_with_mt5(asset, symbol)
                else:
                    logger.info(f"[SYNC] ✓ {asset} positions already in sync ({mt5_type.upper()})")
                    return True
