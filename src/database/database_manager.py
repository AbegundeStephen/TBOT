...
class TradingDatabaseManager:
    ...
    def _check_existing_trade(
        ...
        try:
            if mt5_ticket is not None:
                result = self.supabase.table('trades').select('*').eq('mt5_ticket', mt5_ticket).execute()
                if result.data and len(result.data) > 0:
                    logger.warning(f"[DB] Trade with MT5 ticket {mt5_ticket} already exists (ID={result.data[0]['id']})")
                    return result.data[0]
            
            if binance_order_id is not None:
                result = self.supabase.table('trades').select('*').eq('binance_order_id', binance_order_id).execute()
                if result.data and len(result.data) > 0:
                    logger.warning(f"[DB] Trade with Binance order ID {binance_order_id} already exists (ID={result.data[0]['id']})")
                    return result.data[0]
            
            if position_id is not None:
                result = self.supabase.table('trades').select('*').eq('position_id', position_id).execute()
                if result.data and len(result.data) > 0:
                    logger.warning(f"[DB] Trade with position ID {position_id} already exists (ID={result.data[0]['id']})")
                    return result.data[0]
        ...
        try:
            ...
            if existing_trade:
                if update_if_exists and existing_trade['status'] == 'open':
                    ...
                    if result.data:
                        logger.info(f"[DB] ✓ Trade entry updated: ID={trade_id}, {asset} {side.upper()}")
                        return trade_id, False
                else:
                    logger.info(f"[DB] Trade already exists (ID={existing_trade['id']}, status={existing_trade['status']}), skipping insertion")
            ...
            if result.data and len(result.data) > 0:
                ...
                logger.info(f"[DB] ✓ Trade entry recorded: ID={trade_id}, {asset} {side.upper()}")
    ...
    def update_trade_exit(
        ...
        try:
            update_data = {
                ...
                'status': 'closed',
                'holding_time_hours': float(holding_time_hours) if holding_time_hours else None,
                'final_quantity': float(final_quantity) if final_quantity else None
            }
            
            if metadata:
                existing = self.supabase.table('trades').select('metadata').eq('id', trade_id).execute()
    ...
    def _check_recent_signal(
        ...
        try:
            ...
            result = self.supabase.table('signals').select('*').eq(
                'asset', asset
            ).eq('signal', signal).gte('timestamp', cutoff_iso).execute()
            
            if result.data and len(result.data) > 0:
                logger.debug(f"[DB] Similar signal for {asset} found within {time_window_minutes} minutes")
                return result.data[0]
    ...
    def insert_signal_smart(
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                logger.info(f"[DB] ✓ Signal recorded: ID={signal_id}, {asset} signal={signal}, regime={regime}")
    ...
    def update_signal_execution(self, signal_id: int, executed: bool, trade_id: Optional[int] = None) -> bool:
        ...
        try:
            ...
            result = self.supabase.table('signals').update(update_data).eq('id', signal_id).execute()
    ...
    def insert_daily_summary(
        ...
        try:
            ...
            existing = self.supabase.table('daily_summaries').select('*').eq('date', date_str).execute()
            ...
            if existing.data and len(existing.data) > 0:
                if update_if_exists:
                    result = self.supabase.table('daily_summaries').update(summary_data).eq('date', date_str).execute()
                    if result.data:
                        logger.info(f"[DB] ✓ Daily summary updated: {date_str}, P&L=${total_pnl:.2f}")
                        return True
                else:
                    logger.info(f"[DB] Daily summary for {date_str} already exists, skipping")
                    return True
            else:
                result = self.supabase.table('daily_summaries').insert(summary_data).execute()
                
                if result.data:
                    logger.info(f"[DB] ✓ Daily summary recorded: {date_str}, P&L=${total_pnl:.2f}")
                    return True
    ...
    def get_trade_by_id(self, trade_id: int) -> Optional[Dict]:
        ...
        try:
            result = self.supabase.table('trades').select('*').eq('id', trade_id).execute()
    ...
    def _has_signal_changed(self, asset: str, new_signal_data: Dict) -> bool:
        ...
        for field in key_fields:
            if new_signal_data.get(field) != last_signal.get(field):
                logger.info(f"[DB] Signal changed for {asset}: {field} changed from {last_signal.get(field)} to {new_signal_data.get(field)}")
                return True
        
        # Check if confidence scores changed significantly (>10% change)
        confidence_threshold = 0.1
        confidence_fields = ['signal_quality', 'regime_confidence', 'mr_confidence', 'tf_confidence']
        ...
        for field in confidence_fields:
            ...
            if old_val is not None and new_val is not None:
                if abs(new_val - old_val) > confidence_threshold:
                    logger.info(f"[DB] Signal confidence changed significantly for {asset}: {field} changed by {abs(new_val - old_val):.2%}")
                    return True
    ...
    def get_recent_signals(self, asset: Optional[str] = None, limit: int = 10) -> List[Dict]:
        """Get recent signals"""
        try:
            query = self.supabase.table('signals').select('*').order('timestamp', desc=True).limit(limit)
    ...
    def get_performance_stats(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict:
    ...
    def get_vtm_events_for_trade(self, trade_id: int) -> List[Dict]:
        ...
        try:
            result = self.supabase.table('vtm_events').select('*').eq('trade_id', trade_id).order('timestamp').execute()
            return result.data if result.data else []
...
def calculate_daily_summary_from_trades(db: TradingDatabaseManager, date: datetime) -> bool:
    """
    ...
    try:
        ...
        trades = db.supabase.table('trades').select('*').eq('status', 'closed').gte(
            'exit_time', start_of_day.isoformat()
        ).lte('exit_time', end_of_day.isoformat()).execute()
        ...
        snapshots = db.supabase.table('portfolio_snapshots').select('*').gte(
            'timestamp', start_of_day.isoformat()
        ).lte('timestamp', end_of_day.isoformat()).order('timestamp').execute()
        
        starting_capital = snapshots.data[0]['total_value'] if snapshots.data else 0
        ending_capital = snapshots.data[-1]['total_value'] if snapshots.data else starting_capital
        
        total_pnl_pct = ((ending_capital - starting_capital) / starting_capital * 100) if starting_capital > 0 else 0
