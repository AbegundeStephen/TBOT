...
class HistoricalDataUpdater:
        ...
        try:
            ...
            if csv_path.exists():
                ...
                if not timestamp_col:
                    logger.warning(
                        f"[UPDATE] No timestamp column in {csv_filename}, skipping"
                    )
                    return
                ...
                if pd.isna(last_date):
                    logger.warning(
                        f"[UPDATE] {asset_name} CSV has no valid dates — full backfill"
                    )
                    start_time = None
                else:
            ...
            if isinstance(new_df.index, pd.DatetimeIndex):
                ...
                new_df["date"] = pd.to_datetime(
                    new_df["date"], utc=True, errors="coerce"
                )
                ...
                new_df["date"] = pd.to_datetime(
                    new_df["timestamp"], utc=True, errors="coerce"
                )
            elif "time" in new_df.columns:
                new_df["date"] = pd.to_datetime(
                    new_df["time"], utc=True, errors="coerce"
                )
            else:
                raise ValueError(
            ...
            if existing_df is not None:
                ...
                combined_df = new_df
                logger.info(
                    f"[UPDATE] {asset_name}: Created with {len(combined_df)} candles"
                )
            ...
            columns_to_save = [
                col for col in columns_to_save if col in combined_df.columns
            ]
            ...
            logger.info(
                f"[UPDATE] Date range: {combined_df['date'].min()} to {combined_df['date'].max()}"
            )
    ...
    def verify_data_integrity(self, asset_name):
        ...
        try:
            ...
            logger.info(
                f"  Date range: {df[timestamp_col].min()} to {df[timestamp_col].max()}"
            )
