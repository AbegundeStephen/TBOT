...
class AIVisualizationGenerator:
    ...
    def generate_decision_chart(
        ...
        try:
            ...
            self._plot_main_chart(
                ax_main, df_15min, asset_name, current_price, details
            )
            ...
            signal_color = 'lime' if signal == 1 else 'red' if signal == -1 else 'yellow'
    ...
    def _plot_main_chart(
        ...
        try:
            ...
            if 'date' in df.columns:
                ...
                df['date'] = pd.date_range(end=datetime.now(), periods=len(df), freq='15min')
            
            # Limit to last 100 candles for clarity
            df_plot = df.tail(100).copy()
            
            if len(df_plot) == 0:
                ax.text(0.5, 0.5, 'No candle data available', 
                       ha='center', va='center', fontsize=12)
                ax.set_title(f'{asset_name} - 15min Candlesticks', 
            ...
            if 'ema_50' in df_plot.columns and df_plot['ema_50'].notna().any():
                ax.plot(range(len(df_plot)), df_plot['ema_50'], 
                       color='cyan', linewidth=1.5, label='EMA 50', alpha=0.7)
            if 'ema_200' in df_plot.columns and df_plot['ema_200'].notna().any():
                ax.plot(range(len(df_plot)), df_plot['ema_200'], 
                       color='magenta', linewidth=1.5, label='EMA 200', alpha=0.7)
            
            ax.set_title(f'{asset_name} - 15min Candlesticks (Last 100 candles)', 
                        fontsize=12, fontweight='bold')
    ...
    def _plot_sr_levels(self, ax, df_4h: pd.DataFrame, current_price: float, details: Dict):
        """
        IMPROVED: Plot Support/Resistance levels with defensive checks
        """
        try:
            ...
            if isinstance(ai_details, str):
                ax.text(0.5, 0.5, f'AI Status: {ai_details}', 
                    ha='center', va='center', fontsize=12)
            ...
            if not ai_details or not isinstance(ai_details, dict):
                ax.text(0.5, 0.5, 'AI Validation: Not Available', 
                    ha='center', va='center', fontsize=12)
                ax.set_title('4H Support/Resistance Levels', fontsize=11, fontweight='bold')
                return
            ...
            if not sr_analysis or not sr_analysis.get('levels'):
                ax.text(0.5, 0.5, 'No S/R Levels Found', 
                    ha='center', va='center', fontsize=12)
                ax.set_title('4H Support/Resistance Levels', fontsize=11, fontweight='bold')
                return
            ...
            ax.barh(y_positions, levels_sorted, color=colors, alpha=0.7, edgecolor='white')
            
            # Mark current price
            ax.axvline(x=current_price, color='white', linestyle='--', 
            ...
            if sr_analysis.get('near_sr_level'):
                ...
                if nearest:
                    ax.axvline(x=nearest, color='orange', linestyle='--', 
                            linewidth=2, label=f'Nearest: ${nearest:,.2f}')
            ...
            if isinstance(distance_pct, (int, float)) and distance_pct > 0:
                ax.text(0.02, 0.98, f'Distance to nearest: {distance_pct:.2f}%', 
                    transform=ax.transAxes, va='top', fontsize=9,
                    bbox=dict(boxstyle='round', facecolor='black', alpha=0.5))
            
        except Exception as e:
            logger.error(f"[VIZ] S/R plot error: {e}", exc_info=True)
            ax.text(0.5, 0.5, f'Error displaying S/R levels', 
    ...
    def _plot_pattern_analysis(self, ax, df: pd.DataFrame, details: Dict):
        """
        IMPROVED: Plot pattern detection with defensive checks
        """
        try:
            ...
            if not ai_details or not isinstance(ai_details, dict):
                ax.text(0.5, 0.5, 'Pattern Detection: Not Available', 
                    ha='center', va='center', fontsize=12)
                ax.set_title('AI Pattern Detection', fontsize=11, fontweight='bold')
                ax.axis('off')
                return
            ...
            if not ai_details.get('pattern_detected'):
                ax.text(0.5, 0.5, 'No Pattern Detected', 
                    ha='center', va='center', fontsize=12)
                ax.set_title('AI Pattern Detection', fontsize=11, fontweight='bold')
                ax.axis('off')
                return
            ...
            if top3_patterns and len(top3_patterns) > 0:
                ...
                colors = ['lime' if i == 0 else 'orange' if i == 1 else 'yellow' 
                        for i in range(len(top3_patterns))]
                
                ax.barh(y_pos, top3_confidences, color=colors, alpha=0.7, edgecolor='white')
                ax.set_yticks(y_pos)
            ...
            ax.set_title(f'Pattern: {pattern_name} ({confidence:.1%})', 
                        fontsize=11, fontweight='bold')
            ax.grid(True, alpha=0.3, axis='x')
            
            # Add validation status
            validation_passed = ai_details.get('validation_passed', False)
            validation_status = "[OK] VALIDATED" if validation_passed else "[X] REJECTED"
            status_color = 'lime' if validation_passed else 'red'
            
            ax.text(0.98, 0.02, validation_status, 
            ...
            ax.text(0.5, 0.5, 'Error displaying pattern', 
                ha='center', va='center', fontsize=10)
            ax.set_title('AI Pattern Detection', fontsize=11, fontweight='bold')
            ax.axis('off')
    ...
    def _plot_signal_breakdown(self, ax, details: Dict):
        ...
        try:
            ...
            ax.text(0.5, 0.5, f'Breakdown Error: {str(e)}', 
                   ha='center', va='center')
    ...
    def _plot_ai_validation(self, ax, details: Dict):
        ...
        try:
            ...
            if pattern_detected and pattern_name != 'None':
                ...
                if len(top3) > 1:  # More than just the detected pattern
                    ...
                    for i in range(min(3, len(top3))):
                        if i < len(top3_conf):
                            summary_lines.append(f"    {i+1}. {top3[i]}: {top3_conf[i]:.1%}")
            else:
                summary_lines.append(f"[NO PATTERN]")
                if pattern_conf > 0:
                    summary_lines.append(f"  Highest: {pattern_name} ({pattern_conf:.1%})")
                else:
            ...
            ax.text(0.5, 0.5, f'Error displaying validation:\n{str(e)[:50]}', 
