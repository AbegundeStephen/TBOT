...
class TradingTelegramBot:
    ...
    async def cmd_performance(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            avg_win = sum(t['pnl'] for t in closed_positions if t['pnl'] > 0) / winning_trades if winning_trades > 0 else 0
            avg_loss = sum(t['pnl'] for t in closed_positions if t['pnl'] < 0) / losing_trades if losing_trades > 0 else 0
    ...
    async def cmd_close_all(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ Error closing positions. Check logs."
            )
    ...
    async def cmd_close_asset(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if not context.args:
                await update.message.reply_text(
                    "⚠️ Usage: /close BTC or /close GOLD"
                )
                return
            ...
            if not self.trading_bot.portfolio_manager.has_position(asset):
                await update.message.reply_text(
                    f"ℹ️ No open position for {asset}"
                )
                return
            ...
            if result:
                ...
                await update.message.reply_text(
                    f"❌ Failed to close {asset} position"
                )
    ...
    async def notify_trade_opened(self, asset: str, side: str, price: float, 
    ...
    async def notify_trade_closed(self, asset: str, side: str, pnl: float, 
