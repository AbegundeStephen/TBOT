...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict):
        ...
        logger.info(f"  4H Context: {self.use_4h_context} (Mode: {self.h4_reversion_mode})")
        if self.use_4h_context:
            logger.info(f"  4H Extreme Weight: {self.h4_extreme_weight}, Trend Penalty: {self.h4_trend_penalty}")
    ...
    def _align_4h_to_1h(self, df_1h: pd.DataFrame, df_4h: pd.DataFrame) -> pd.DataFrame:
        ...
        h4_features = ['bb_position', 'rsi', 'stoch_k', 'adx', 
                       'bb_width_norm', 'atr_pct', 'close']
    ...
    def _calculate_4h_reversion_context(self, df_4h_aligned: pd.DataFrame, idx: int) -> dict:
        """
    ...
    def generate_labels(self, df: pd.DataFrame, df_4h: pd.DataFrame = None) -> pd.Series:
        ...
        for i in range(len(df) - self.reversion_window - 1):
            ...
            # === GET 4H CONTEXT ===
            h4_context = {'is_extreme_oversold': False, 'is_extreme_overbought': False,
                         'is_trending': False, 'trend_direction': 0, 'reversion_score': 0.0}
