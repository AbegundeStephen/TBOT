...
class ErrorContext:
    ...
    def __post_init__(self):
        ...
        error_signature = f"{self.component}:{type(self.exception).__name__}:{self.message}"
...
class ErrorAggregator:
    ...
    def __init__(self, window_seconds: int = 300, max_duplicate_notifications: int = 3):
        ...
        self.error_occurrences: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
    ...
    def should_notify(self, error_ctx: ErrorContext) -> tuple[bool, Optional[str]]:
        ...
        while (self.error_occurrences[error_hash] and 
               self.error_occurrences[error_hash][0] < cutoff):
        ...
        if self.notification_count[error_hash] >= self.max_duplicate_notifications:
            ...
            if last_notif and (now - last_notif).total_seconds() < self.window_seconds:
                ...
                return False, f"(Suppressed: {occurrence_count} occurrences in {self.window_seconds}s)"
...
class GlobalErrorHandler:
    ...
    def __init__(self, telegram_bot=None, db_manager=None, config: Dict = None):
        ...
        self.aggregator = ErrorAggregator(
            ...
            max_duplicate_notifications=self.config.get("max_duplicate_notifications", 3)
        )
    ...
    def _send_notification(self, error_ctx: ErrorContext):
        ...
        try:
            ...
            if not should_notify:
                logger.debug(f"[ERROR HANDLER] Suppressing duplicate notification for {error_ctx.component}")
                return
            ...
            if hasattr(self.telegram_bot, '_send_telegram_notification'):
                ...
                logger.warning("[ERROR HANDLER] Telegram bot doesn't have notification method")
        
        except Exception as e:
            logger.error(f"[ERROR HANDLER] Failed to send notification: {e}")
    
    def _format_telegram_message(self, error_ctx: ErrorContext, aggregation_msg: Optional[str]) -> str:
        ...
        if error_ctx.additional_info:
            ...
            for key, value in list(error_ctx.additional_info.items())[:5]:  # Limit to 5 items
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                elif args and hasattr(args[0], 'trading_bot') and hasattr(args[0].trading_bot, 'error_handler'):
                ...
                if error_handler:
                    ...
                    logger.error(f"[{component}] Error in {func.__name__}: {e}", exc_info=True)
