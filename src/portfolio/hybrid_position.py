...
class HybridPositionSizer:
    ...
    def _apply_confidence_adjustment(
        ...
        adjustments.append(
            f"market_condition={market_condition}_{condition_scalar:.2f}x"
        )
    ...
    def _apply_hard_limits(
        ...
        if position_size < min_size:
            logger.debug(
                f"Position size ${position_size:,.2f} below minimum ${min_size}, skipping"
            )
            return 0.0
    ...
    def generate_sizing_report(self) -> Dict:
        ...
        automated_count = sum(
            1 for h in self.override_history if h["mode"] == "AUTOMATED"
        )
        ...
        return {
            ...
            "override_pct": (
                (override_count / len(self.override_history) * 100)
