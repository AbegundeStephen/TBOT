...
class DataManager:
        ...
        try:
            ...
            if not api_key or not api_secret or api_key == 'YOUR_BINANCE_API_KEY':
                ...
                if api_config.get('testnet', True):
                    ...
                    self.binance_client.API_URL = api_config.get('api_base', 'https://testnet.binance.vision/api')  
                else:
    ...
    def initialize_mt5(self) -> bool:
        ...
        try:
            ...
            if 'api' not in self.config or 'mt5' not in self.config['api']:
                logger.warning("MT5 configuration not found in config. Skipping MT5 initialization.")
                return False
            ...
            if not all([login, password, server]):
                logger.warning("MT5 credentials incomplete. Skipping MT5 initialization.")
                return False
            ...
            logger.info(f"MT5 Python Package Version: {mt5.__version__ if hasattr(mt5, '__version__') else 'Unknown'}")
            
            logger.info("\nStep 1: Initializing MT5 terminal...")
            
            logger.info("Attempting initialize without path (auto-detect running MT5)...")
            ...
            if not authorized:
                ...
                logger.error("3. Server name incorrect (check in MT5: Tools > Options > Server)")
                logger.error("4. Internet connection issue")
                
                account_info = mt5.account_info()
                if account_info:
                    logger.info(f"Current account: {account_info.login} (trying to switch)")
            ...
            if symbol_info is None:
                ...
                logger.warning("This might be normal if symbol doesn't exist on this account")
            else:
    ...
    def _fetch_klines_with_retry(self, symbol: str, interval: str, startTime: int, endTime: int, limit: int):
        ...
        try:
            ...
            while current_start < end_ts and iteration < max_iterations:
                ...
                if last_timestamp <= current_start:
                    logger.warning(f"Duplicate timestamp detected, stopping to prevent infinite loop")
                    break
                
                # Move to next batch (add 1ms to avoid duplicates)
                current_start = last_timestamp + 1

                logger.info(f"Batch {iteration}: Fetched {len(klines)} bars, total: {len(all_klines)}")
            ...
            if not all_klines:
                logger.error(f"No data received for {symbol} from {start_date} to {end_date}")
                return pd.DataFrame()
            ...
            logger.info(f"Coverage: {days_received}/{days_requested} days ({days_received/max(days_requested,1)*100:.1f}%)")
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed > 0:
            logger.info(f"Cleaned data: removed {removed} bars ({removed/initial_len*100:.1f}%)")
