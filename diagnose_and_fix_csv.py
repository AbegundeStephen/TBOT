...
def fix_csv_smart(input_file: str, output_file: str = None) -> bool:
    ...
    try:
        ...
        if not has_date:
            logger.warning("⚠ No valid timestamp/date column - will create index-based date")
            df['date'] = pd.date_range(start='2020-01-01', periods=len(df), freq='15min')
        ...
        if len(df) < 100:
            logger.error(f"❌ Insufficient data after cleaning: {len(df)} rows (need ≥100)")
            return False
        ...
        logger.info(f"  Removed: {initial_rows - len(df)} ({(initial_rows - len(df))/initial_rows*100:.1f}%)")
