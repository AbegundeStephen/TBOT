...
def fetch_btc_data(data_manager: DataManager, config: dict) -> pd.DataFrame:
    ...
    logger.info(f"Start Date: {start_date.strftime('%Y-%m-%d')} (going back {config['lookback_days']} days)")
...
def train_asset_strategies(asset_key: str, train_df: pd.DataFrame, config: dict) -> dict:
    ...
    logger.info(f"    BB: {mr_config['bb_lower_threshold']}/{mr_config['bb_upper_threshold']}")
    ...
    try:
        ...
        if mr_metrics.get('success'):
            ...
            logger.info(f"   CV Accuracy: {mr_metrics.get('cv_mean_accuracy', 0):.2%} ± {mr_metrics.get('cv_std_accuracy', 0):.2%}")
    ...
    try:
        ...
        if tf_metrics.get('success'):
            ...
            logger.info(f"   CV Accuracy: {tf_metrics.get('cv_mean_accuracy', 0):.2%} ± {tf_metrics.get('cv_std_accuracy', 0):.2%}")
...
def calculate_data_correlation(btc_df: pd.DataFrame, gold_df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if isinstance(gold_close.index, pd.DatetimeIndex) and gold_close.index.tz is not None:
            gold_close.index = gold_close.index.tz_localize(None)
        
        # Now merge on timezone-naive indices
        merged = pd.merge(btc_close, gold_close, left_index=True, right_index=True, how='inner')
        
        if len(merged) < 50:
            logger.warning(f"Insufficient overlapping data for correlation: {len(merged)} bars")
            return None
...
def main():
    ...
    if not btc_df.empty and validate_data_quality(btc_df, 'BTC', btc_fetch_config['min_bars_required']):
    ...
    if data_manager.mt5_initialized:
        ...
        if not gold_df.empty and validate_data_quality(gold_df, 'GOLD', gold_fetch_config['min_bars_required']):
    ...
    if 'BTC' in asset_data and 'GOLD' in asset_data:
        ...
        logger.info("="*70)
        correlation_stats = calculate_data_correlation(asset_data['BTC'], asset_data['GOLD'])
    else:
