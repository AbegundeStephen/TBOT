...
- **Phase 3 — CMR rewrite + phase_config wiring fix (2026-06-27). CODE COMPLETE, needs `py_compile` + backtest + restart.** All in `council_aggregator.py` + `config/aggregator_presets.json`, flag-gated and reversible:
  1. **Candle Momentum Reversal (CMR) rewritten — veteran logic.** `_check_recent_momentum_alignment` no longer votes on candle COLOUR alone. It now flags a trend-aligned entry as adverse only when BOTH (a) `min_agreement`/`candles` recent CLOSED candles oppose the trade AND (b) NET close-to-close displacement moves against the trade by ≥ `adverse_atr_mult` × fast-ATR — i.e. price has actually travelled against the entry, not just printed a couple of small opposite bodies. This fixes the core flaw (two tiny green + one big red netting DOWN still blocked a SELL) and subsumes the old average-body doji filter. The caller no longer hard-vetoes: new `momentum_alignment.mode` = `"soft"` (default) requires `total_score ≥ required_score + soft_penalty` to proceed (high-conviction council survives a bad-timing bounce; a marginal one stands down); a strong-ADX trend (`adx ≥ strong_adx`) HALVES the penalty because a counter-move in a strong trend is a pullback-continuation entry, not a reversal. `"veto"` = legacy-style hard block (but now needs real displacement); `"off"` disables. New tunables in the 6 enabled BTC/GOLD presets: `mode`, `adverse_atr_mult` (1.0), `strong_adx` (28.0), `soft_penalty` (0.75); presets without a block (FX-routed council assets) get these via code defaults. **Revert to legacy:** set `mode:"veto"` + `adverse_atr_mult:0.0`. Shadow corroboration: a GOLD short blocked by CMR went +2.69% (n=1 — directional, validate on backtest). NOTE the live preset was already `min_agreement:3` (unanimous), not 2 — earlier read was from the wrong file.
  2. **🔧 phase_config WIRING BUG fixed (made the 6/24 counter-trend cap actually work).** The council's `self.config` is the aggregator PRESET and never carries `phase_config`, so `self.config.get("phase_config")` was always `{}`. The 6/24 counter-trend cap and MR-lean gate read that dead path: the **counter-trend cap=3.5 was a silent no-op** (pinned at the 4.0 code default), and MR-lean "soft" only worked because its code defaults coincidentally matched config. Both now read `getattr(_composite_state, "phase_config", {})` — the live source the rr-gate/lifecycle gates already use. **Counter-trend cap=3.5 now genuinely applies** (this is a real behavior change — re-backtest). ⚠️ Latent: the `lsm_regime_disagreement` gate (≈L1807) and the down-weight gate (≈L2791) still read the dead `self.config` path — left as-is (default-off; flipping them would change behavior silently) — fix when intentionally enabling them.
- **Phase 3 — FIRST ACTIONS (2026-06-24, from funnel+shadow soak 6/20–6/24). CODE COMPLETE, needs `py_compile` + backtest + restart.** Two gates relaxed in `council_aggregator.py`, both flag-gated and reversible:
  1. **MR-lean-conflict gate relaxed.** New `phase_config.council_mr_lean_mode`: `"soft"` (default) | `"off"` | `"strict"`. The veto was firing on the council's strongest signals — GOLD/USOIL/BTC MAIN_DOWN/UP trend signals scoring 4.0–4.3 were blocked at the legacy bar (2.75 + 1.5 = 4.25), and the shadow engine forward-tracked several of those as winners (USOIL short +2.03%, GOLD short +2.69%). `"soft"` uses tunable bumps `council_mr_lean_bump_main` (0.25) / `council_mr_lean_bump_secondary` (0.0) → MAIN bar drops to ~3.0; `"off"` removes the gate entirely (council still reads `livermore_state_1h` directly via the trend-judge confirmation, lifecycle guard, and regime-disagreement gate, so structure is NOT lost); `"strict"` restores the legacy 1.5/0.5 hard veto.
  2. **Counter-trend ceiling capped.** New `phase_config.council_counter_trend_cap` (config **3.5**, code fallback 4.0 = legacy). The effective counter-trend bar was climbing to 4.0 (base 3.5 + Livermore RSM +0.5), rejecting BTC counter-trend signals that scored exactly 3.50. Applied to `_effective_counter_threshold` and the SLIGHTLY_COUNTER `min(...)` raise. Trend-aligned bar untouched.
  - **Scope:** council only (BTC/GOLD/USOIL/USDJPY/GBPUSD route through it; the MR-lean gate and the `/4.0` bar are council-exclusive — the performance aggregator uses 0.25/0.55-style thresholds and was not touched).
  - **Revert:** set `council_mr_lean_mode:"strict"` and `council_counter_trend_cap:4.0` to restore pre-6/24 behaviour exactly.
  - **Before deploy:** `py_compile config/config.json (json), council_aggregator.py`; backtest with the relaxed vs strict configs; expect more counter-trend + trend-conflict entries — confirm the added trades don't degrade expectancy before going live. Sample still thin (mostly n=1–5 per gate); let the soak keep accumulating.
...
- **Phase 2.4 — DONE (durable shadow engine).** The existing `ShadowTradingEngine` already tracked blocked signals' forward P&L by gate (`get_gate_scorecard`), but was wiped on every restart (no load-on-startup; snapshot-only dump). Now: `_archive` appends every closed shadow trade to an append-only `logs/shadow/closed_<date>.jsonl`, and `load_state()` (called at startup, `trading.shadow_archive_lookback_days` default 30) restores the history so the scorecard accumulates across restarts and can reach the 200-trade calibration threshold. Load+scorecard logic unit-tested.
  ...
  - **Future enhancement → DONE 2026-06-20 (AI A/B wiring).** AI-rejected signals now flow to the shadow engine: when the council's AI validator actively zeroes a real signal (`ai_modified` + final 0), main.py opens a shadow position for the pre-veto direction (`original_signal`) tagged gate `ai_validation`, throttled by the shadow engine's existing per-asset+side dedup + 60-min cooldown. So the AI filter's forward P&L is now measurable like any other gate (look for the `ai_validation` row in the shadow scorecard). Also tightened both the funnel classifier and ai_rejects log to key off `ai_modified` instead of the unreliable `validation_passed` (which several paths define as just `signal != 0`) — this fixes a misclassification where score/quality rejections were being counted as AI rejections (the early "14/14 BTC AI-rejected" read was likely contaminated by that). Classifier logic unit-tested. Needs the usual `py_compile` + restart; safe to deploy mid-soak (observational only).
