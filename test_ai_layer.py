...
def load_trained_model():
    ...
    sniper = OHLCSniper(input_shape=(15, 4), num_classes=config["num_classes"])
...
def test_integration():
    ...
    for test in test_signals:
        signal = test["signal"]
        signal_details = {"reasoning": "test_signal", "signal_quality": 0.75}

        logger.info(f"\nTesting {test['name']} signal...")

        final_signal, enhanced_details = validator.validate_signal(
            signal=signal, signal_details=signal_details, df=df
        )
        ...
        if "ai_pattern_check" in enhanced_details:
            ...
            logger.info(
                f"  Pattern: {pat.get('pattern_name', 'N/A')} ({pat.get('confidence', 0):.0%})"
            )
